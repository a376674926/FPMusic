...
import com.android.music.base.BaseListActivity;
...
public class TrackBrowserActivity extends BaseListActivity
        implements View.OnCreateContextMenuListener, MusicUtils.Defs, ServiceConnection,BaseListActivity.BottomKeyClickListener
{
    ...
    private static boolean mHasAlbumImg = false;
    ...
    public void onCreate(Bundle icicle)
    {
        ...
        mEmptyTextView = (TextView) mRootView.findViewById(R.id.empty);
        ...
        mTrackList.setOnItemSelectedListener(new OnListItemSelectListener());
        if (mEditMode) {
            ((TouchInterceptor) mTrackList).setDropListener(mDropListener);
            ((TouchInterceptor) mTrackList).setRemoveListener(mRemoveListener);
            ((TouchInterceptor) mTrackList).registerContentObserver(getApplicationContext());
//            mTrackList.setSelector(R.drawable.list_selector);
        } else {
//            mTrackList.setSelector(R.drawable.list_item_selector);
    ...
    public void init(Cursor cursor, boolean isLimited) {
        if (mAdapter == null) {
            return;
        }
        ...
        // mAdapter.changeCursor(cursor); // also sets mTrackCursor
        if (mPlaylist == null) {
            ArrayList<MusicInfo> infos = new ArrayList<MusicInfo>();
            while (cursor.moveToNext()) {
                MusicInfo info = new MusicInfo();
                info.mAudioId = cursor.getInt(cursor.getColumnIndex(MediaStore.Audio.Media._ID));
                info.mTitle = cursor.getString(cursor.getColumnIndex(MediaStore.Audio.Media.TITLE));
                info.mData = cursor.getString(cursor.getColumnIndex(MediaStore.Audio.Media.DATA));
                info.mArtist = cursor.getString(cursor
                        .getColumnIndex(MediaStore.Audio.Media.ARTIST));
                info.mArtistId = cursor.getInt(cursor
                        .getColumnIndex(MediaStore.Audio.Media.ARTIST_ID));
                info.mALBUM = cursor.getString(cursor.getColumnIndex(MediaStore.Audio.Media.ALBUM));

                int isMusicIdx = cursor.getColumnIndex(MediaStore.Audio.Media.IS_MUSIC);
                if (isMusicIdx >= 0) {
                    info.mISMUSIC = cursor.getInt(isMusicIdx);
                }
                info.mDuration = cursor.getInt(cursor
                        .getColumnIndex(MediaStore.Audio.Media.DURATION));
                infos.add(info);
            }
            Collections.sort(infos, new CollatorComparator());
            Cursor newCursor = createCursor(infos);
    ...
    private void setAlbumArtBackground() {
        if (!mEditMode) {
            try {
                ...
                if (bm != null) {
                    Bitmap albumImg = MusicUtils.getArtworkFromFile(getApplicationContext(), -1, albumid);
                    mHasAlbumImg = albumImg != null ? true :false;
                    TrackListAdapter adapter = (TrackListAdapter)mTrackList.getAdapter();
        ...
        if (mAlbumId != null) {
            ...
            if (numresults > 0) {
                ...
                if (cursor != null) {
                    ...
                    cursor.deactivate();
                    // add begin by lzy@20141005,for close cursor.
                    if (cursor != null) {
                        cursor.close();
                    }
                    // add end.
    ...
    public void onCreateContextMenu(ContextMenu menu, View view, ContextMenuInfo menuInfoIn) {
       ...
       // MusicUtils.makePlaylistMenu(this, sub);
        ...
        if (MusicUtils.isMultiSimCard(getBaseContext())) {
    ...
    private class OnListItemSelectListener implements OnItemSelectedListener{
    ...
    {
        doListItemClick(position) ;
    }

    private void doListItemClick(int position) {
        ...
        if (mPlaylist != null && mEditMode && !mPlaylist.equals("nowplaying")) {
    ...
    // add begin by lzy@20150727,for bug:6261
    // 1.Music list support mixed sorting in Chinese and English.
    // 2.When move the fast scroll indexer to Chinese don't show the alpha.
    ...
    private Cursor createCursor(ArrayList<MusicInfo> infos) {
        ...
        for (MusicInfo info : infos) {
            cursor.addRow(new Object[] {
                    info.mAudioId, info.mTitle, info.mData, info.mArtist, info.mArtistId,
                    info.mALBUM, info.mISMUSIC, info.mDuration
            });
        }
        return cursor;
    }

    private class CollatorComparator implements Comparator<MusicInfo> {

        @Override
        public int compare(MusicInfo element1, MusicInfo element2) {

            String str1 = Cn2Spell.CN2FirstCharSpell(element1.mTitle);
            String str2 = Cn2Spell.CN2FirstCharSpell(element2.mTitle);
    ...
    public void onLeftKeyPress() {
        doListItemClick(mSelectedPosition);
    }
